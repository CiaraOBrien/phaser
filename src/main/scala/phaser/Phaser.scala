...
sealed trait Phaser[A : Type](using Quotes) {
  ...
  def requireStatic(message: String): Static[A] = this match
    case a: Static[A]  => a
    case d: Dynamic[A] => report.error(message); ???
  ...
  def autoProduct: Phaser[(A, A)] = this match 
    case Static (a) => Static((a, a))
    case Dynamic(e) => Dynamic('{($e, $e)})
  
  def validate(pred: Expr[A] => Boolean, reason: String)(using ToExpr[A]): Phaser[A] = 
    this.defer.validate(pred, reason)
    this
...
object Phaser {
  ...
  def require[A : Type : FromExpr](e: Expr[A], desc: String, validate: A => Boolean = ((a: A) => true), as: String = "failed to validate")(using Quotes): Static[A] = 
  ...
  def phase[A : Type](e: Expr[A])(using Quotes, FromExpr[A]): Phaser[A] = e.value.map(Static.apply).getOrElse(Dynamic(e))
}
...
final case class Static[A : Type](val value: A)(using Quotes) extends Phaser[A] {
  ...
  def validate(pre: A => Boolean, reason: String): Static[A] = pre(value) match
    case true => this
    case _ => quotes.reflect.report.error(s"$reason: $value."); ???

}

final case class Dynamic[A : Type](val expr: Expr[A])(using Quotes) extends Phaser[A] {
  
  override def toString: String = s"Dynamic($clippedShow)"
  ...
  def validate(pred: Expr[A] => Boolean, reason: String): this.type = pred(expr) match
    case true => this
    case _ => quotes.reflect.report.error(s"$reason: $clippedShow."); ???

  def clippedShow: String = if expr.show.length <= 100 then expr.show else expr.show.substring(0, 97) + "..."
